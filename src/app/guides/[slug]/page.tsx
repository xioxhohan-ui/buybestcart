...
import { generateArticleJsonLd, generateFaqJsonLd, generateGuideProductsJsonLd, generateBreadcrumbJsonLd } from '@/lib/seo';
...
export async function generateMetadata({ params }: GuidePageProps): Promise<Metadata> {
  ...
  if (!article || article.status !== 'published') {
    return {
      title: 'Guide Not Found | Buy Best Cart',
  ...
  const ogImageUrl = article.og_image || article.featured_image || `${CANONICAL_BASE_URL}/og-image.png`;
...
export default async function GuideDetailPage({ params }: GuidePageProps) {
  ...
  if (!article || article.status !== 'published') {
    const redirectRecord = await getRedirectForPath(`/guides/${slug}`);
  ...
  return (
    <article className="container" style={{ paddingTop: '1.5rem', paddingBottom: '5rem', maxWidth: '880px' }}>
      ...
      <header style={{ marginBottom: '2.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '1.75rem' }}>
        ...
        {/* Amazon Associates Permanent Disclosure Banner */}
        <div
          style={{
            marginTop: '1.25rem',
        ...
        >
          <div>
            <strong>Affiliate Disclosure:</strong> Buy Best Cart is reader-supported. As an Amazon Associate I earn from qualifying purchases at no extra cost to you.
          </div>
          <Link
            href="/affiliate-disclosure"
            style={{ color: 'var(--green-accent)', fontWeight: 700, textDecoration: 'underline', textUnderlineOffset: '2px', whiteSpace: 'nowrap' }}
          >
            Full Disclosure →
          </Link>
        </div>
