          ...
          <div style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', paddingTop: '1.25rem', borderTop: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: 'var(--bg-subtle)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--green-accent)', fontSize: '0.75rem', fontWeight: 800 }}>
                  <User size={14} />
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-primary)' }}>
                    {featuredArticle.author_name || 'Editorial Staff'}
                  </div>
                  <div style={{ fontSize: '0.6875rem', color: 'var(--text-muted)' }}>
                    {featuredArticle.author_role || 'Senior Tech Analyst'}
                  </div>
                </div>
              </div>

              <Link href={`/guides/${featuredArticle.slug}`} className="btn btn-primary btn-sm" style={{ gap: '0.35rem' }}>
            ...
            >
              ...
              <Link href={`/guides/${article.slug}`} style={{ position: 'relative', display: 'block', aspectRatio: '16/9', overflow: 'hidden', background: '#0F172A' }}>
                ...
                {topCount > 0 && (
                  ...
                  >
                    <Sparkles size={11} color="var(--amber-deal)" />
              ...
              <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.65rem' }}>
                  <span style={{ fontSize: '0.6875rem', fontWeight: 800, color: 'var(--green-accent)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    {article.category?.name || (article.content_type === 'buying_guide' ? 'Buying Guide' : 'Product Review')}
                  </span>
