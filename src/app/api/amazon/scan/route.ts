...
import { signAmazonPaApiRequest } from '@/lib/api/amazonSigV4';

export async function POST(request: NextRequest) {
  try {
    const cookieToken = request.cookies.get('bbc_admin_session')?.value;
    ...
    const { data: apiSettings } = await supabase.from('settings').select('value').eq('key', 'api_configs').maybeSingle();
    ...
    if (hasCredentials) {
      try {
        const host = `webservices.${domain}`;
        const endpoint = `https://${host}/paapi5/getitems`;
        const target = 'com.amazon.paapi5.v1.AmazonProductAdvertisingAPIv1.GetItems';
        const region = amazonConfig.region || (domain.endsWith('.co.uk') || domain.endsWith('.de') || domain.endsWith('.fr') || domain.endsWith('.it') || domain.endsWith('.es') ? 'eu-west-1' : domain.endsWith('.co.jp') ? 'us-west-2' : 'us-east-1');
        ...
        const signedHeaders = signAmazonPaApiRequest({
          accessKey: amazonConfig.apiKey,
        ...
        // Attempt official Creators API HTTP request with SigV4 headers
