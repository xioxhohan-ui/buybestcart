...
export default function AdminComparisonsPage() {
  ...
  const fetchData = async () => {
    ...
    const [compRes, prodRes] = await Promise.all([
      ...
      supabase.from('products').select('id, title, slug, price, thumbnail_url, rating').in('status', ['active', 'featured', 'published']),
    ]);
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                ...
                <SlugUrlAdvisor
                  ...
                  currentId={editingComparison?.id}
              ...
              <SeoMetadataEditor
                ...
                currentId={editingComparison?.id}
                onAutoGenerate={() => {
                  const prodA = products.find((p) => p.id === formData.product_a_id);
                  const prodB = products.find((p) => p.id === formData.product_b_id);
                  const auto = generateComparisonMetadata(formData, prodA?.title, prodB?.title);
                  setFormData({
                    ...formData,
