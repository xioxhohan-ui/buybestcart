...
import { scanContent, scanUrl } from '@/lib/compliance/scanner';
...
export default function AdminDealsPage() {
  ...
  const complianceResult: ComplianceScanResult = React.useMemo(() => {
    const violations: ComplianceViolation[] = [];
    if (formData.title) violations.push(...scanContent(formData.title, { field: 'title' }));
    if (formData.description) violations.push(...scanContent(formData.description, { field: 'description' }));
    if (formData.cta_text) violations.push(...scanContent(formData.cta_text, { field: 'cta_text' }));
    if (formData.cta_url) violations.push(...scanUrl(formData.cta_url, { field: 'cta_url' }));

    const criticalCount = violations.filter((v) => v.severity === 'critical').length;
    const highCount = violations.filter((v) => v.severity === 'high').length;
    let score = 100 - criticalCount * 30 - highCount * 15;
    ...
    return {
      ...
      totalChecks: 4,
      violations,
      hasBlockingViolations: criticalCount > 0 || highCount > 0,
      criticalCount,
      highCount,
      ...
      targetType: 'deal',
      targetId: editingDeal?.id,
      targetTitle: formData.title,
    };
  }, [formData, editingDeal?.id]);
  ...
  const openAddModal = () => {
    ...
    setFormData({
      ...
      deal_label: 'Featured Value Highlight',
      current_price: '',
      previous_price: '',
      savings_percentage: '',
  ...
  const openEditModal = (d: Deal) => {
    setEditingDeal(d);
    const dealPrice = (d as any).deal_price ?? d.current_price;
    const origPrice = (d as any).original_price ?? d.previous_price;
    const discountPct = (d as any).discount_percentage ?? d.savings_percentage;

    setFormData({
      title: d.title,
      slug: d.slug || '',
      product_id: d.product_id || '',
      deal_label: d.deal_label || d.badge || 'Limited Time Deal',
      current_price: dealPrice ? dealPrice.toString() : '',
      previous_price: origPrice ? origPrice.toString() : '',
      savings_percentage: discountPct ? discountPct.toString() : '',
  ...
  const handleSaveAsDraft = async () => {
    ...
    const payload = {
      ...
      deal_price: cur,
      original_price: prev || null,
      discount_percentage: calcSavings,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (formData.status === 'active' && complianceResult.hasBlockingViolations) {
      ...
      try {
        fetch('/api/compliance/audit', {
          ...
          body: JSON.stringify({
            rule_id: complianceResult.violations[0]?.ruleId || 'AMZ-RULE-08',
            rule_number: complianceResult.violations[0]?.ruleNumber || 8,
            rule_title: complianceResult.violations[0]?.title || 'Deal Compliance Violation',
            severity: complianceResult.violations[0]?.severity || 'critical',
            affected_item: formData.title,
            affected_type: 'deal',
            action: 'publish_blocked',
            status: 'open',
            details: `Deal publish blocked due to ${complianceResult.criticalCount} critical violations`,
            offending_value: complianceResult.violations[0]?.offendingValue,
          }),
        });
      } catch {}
      return;
    ...
    if (editingDeal) {
      ...
      if (!error) {
        if (payload.product_id) {
          await supabase.from('products').update({ is_deal: payload.status === 'active', show_in_deals: payload.status === 'active' }).eq('id', payload.product_id);
        }
        setShowModal(false);
        fetchData();
        triggerRevalidation();
  ...
  return (
    <div>
      ...
      <div className="admin-table-wrapper">
        <table className="editorial-table" style={{ minWidth: '760px' }}>
          <thead>
            <tr>
              <th>Priority</th>
          ...
          >
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 700, marginBottom: '0.35rem' }}>
                    Deal Price ($) <span style={{ fontWeight: 400, color: 'var(--text-muted)' }}>(Optional)</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
