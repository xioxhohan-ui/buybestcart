...
import { scanProduct, scanUrl } from '@/lib/compliance/scanner';
...
export default function AdminAmazonPage() {
  ...
  const complianceResult = React.useMemo(() => {
    ...
    return scanProduct({
      ...
      amazon_url: editingItem.amazon_url,
      affiliate_url: editingItem.affiliate_url,
  ...
  const fetchAmazonProducts = async () => {
    ...
    if (prodRes.data) {
      const formatted: AmazonProduct[] = prodRes.data.map((p) => ({
        ...
        price: p.price ? `$${p.price}` : 'Price not set',
        currency: p.currency || 'USD',
        availability: p.availability || 'In Stock',
        rating: p.rating || 4.8,
        amazon_url: p.amazon_url || (p.asin ? `https://www.amazon.com/dp/${p.asin}?tag=bestbuycart-20` : ''),
        affiliate_url: p.affiliate_url || p.amazon_url || (p.asin ? `https://www.amazon.com/dp/${p.asin}?tag=bestbuycart-20` : ''),
  ...
  const handleSaveProduct = async (e?: React.FormEvent, overrideStatus?: 'published' | 'draft') => {
    if (e) e.preventDefault();
    if (!editingItem) return;

    const rawPrice = editingItem.price ? parseFloat(editingItem.price.toString().replace(/[^0-9.]/g, '')) : null;
    const targetStatus = overrideStatus || editingItem.status || 'published';

    // Real-Time Compliance Guard Check
      ...
      try {
        fetch('/api/compliance/audit', {
          ...
          body: JSON.stringify({
            ...
            rule_title: scanRes.violations[0]?.title || 'Scanned Amazon Item Violation',
            severity: scanRes.violations[0]?.severity || 'critical',
            affected_item: editingItem.title,
            affected_type: 'amazon_product',
            action: 'publish_blocked',
            status: 'open',
            details: `Publishing blocked due to ${scanRes.criticalCount} critical violations`,
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        title: editingItem.title,
        manufacturer: editingItem.brand,
        price: rawPrice && !isNaN(rawPrice) ? rawPrice : null,
  ...
  return (
    <div style={{ paddingBottom: '4rem' }}>
                  ...
                  filteredProducts.map((prod) => (
                    <tr key={prod.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '0.85rem 1rem' }}>
                        ...
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{formatAvailability(prod.availability)}</div>
                      </td>
          ...
          >
            ...
            <form onSubmit={handleSaveProduct} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 700, marginBottom: '0.35rem' }}>
                    Price ($ USD) <span style={{ fontWeight: 400, color: 'var(--text-muted)' }}>(Optional)</span>
                  </label>
                  <input
                    type="text"
      ...
      <ComplianceScanModal
        ...
        onSaveAsDraft={() => handleSaveProduct(undefined, 'draft')}
        savingAsDraft={savingProduct}
      />
    </div>
  );
}
