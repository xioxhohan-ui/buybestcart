...
import { scanArticle } from '@/lib/compliance/scanner';
...
export default function AdminBlogGuidesPage() {
  ...
  const [formData, setFormData] = useState<{
    ...
    excerpt: '',
    introduction: '',
    how_we_tested: undefined,
    what_to_look_for: undefined,
    faqs: [],
    content: '',
    featured_image: '',
    status: 'published',
    ...
    author_role: 'Senior Hardware Analyst',
    author_avatar: '',
    tags: [],
    seo_title: '',
    seo_description: '',
  ...
  const handleSave = async (overrideStatus?: ArticleStatus) => {
    ...
    // Real-Time Amazon Compliance Pre-Publish Scan
    const scanRes = scanArticle({
      ...formData,
      id: editingGuide?.id,
    });

    if (targetStatus === 'published' && scanRes.hasBlockingViolations) {
      ...
      try {
        fetch('/api/compliance/audit', {
          ...
          body: JSON.stringify({
            rule_id: scanRes.violations[0]?.ruleId || 'AMZ-RULE-06',
            rule_number: scanRes.violations[0]?.ruleNumber || 6,
            rule_title: scanRes.violations[0]?.title || 'Guide Compliance Violation',
            severity: scanRes.violations[0]?.severity || 'critical',
            affected_item: formData.title,
            affected_type: 'article',
            action: 'publish_blocked',
            status: 'open',
            details: `Publishing blocked due to ${scanRes.criticalCount} critical violations in guide content`,
    ...
    setSaving(true);
    const cleanSlug = generateCleanSlug(formData.slug || formData.title);
    ...
    if (editingGuide) {
      if (editingGuide.slug && editingGuide.slug !== cleanSlug) {
        await recordSlugChangeRedirect(`/guides/${editingGuide.slug}`, `/guides/${cleanSlug}`);
      }
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                {activeTab === 'content' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    ...
                    <SlugUrlAdvisor
                      ...
                      routePrefix="/guides"
                      tableName="articles"
                      currentId={editingGuide?.id}
                      onChange={(newSlug) => setFormData({ ...formData, slug: newSlug })}
                    />

                    {/* Featured Image & Universal Embedder */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      <EmbedImageInput
                        label="Embed Image HTML / Image Link (Featured or Article)"
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', marginBottom: '0.35rem' }}>
                          Direct Featured Cover Image URL
                        </label>
                        <input
                          type="url"
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '0.25rem', background: '#F1F5F9', border: '1px solid var(--border)', borderBottom: 'none', borderRadius: 'var(--radius-xs) var(--radius-xs) 0 0', padding: '0.35rem 0.5rem', flexWrap: 'wrap' }}>
                        <button type="button" onClick={() => handleInsertSnippet('\n## Section Heading\n\n')} className="btn btn-secondary btn-sm" style={{ padding: '0.15rem 0.4rem', fontSize: '0.6875rem', fontWeight: 700 }} title="H2 Heading">
                          H2
                        </button>
                        <button type="button" onClick={() => handleInsertSnippet('\n### Sub-Heading\n\n')} className="btn btn-secondary btn-sm" style={{ padding: '0.15rem 0.4rem', fontSize: '0.6875rem', fontWeight: 700 }} title="H3 Heading">
                          H3
                        </button>
                        <button type="button" onClick={() => handleInsertSnippet('\n#### Detailed Topic\n\n')} className="btn btn-secondary btn-sm" style={{ padding: '0.15rem 0.4rem', fontSize: '0.6875rem', fontWeight: 700 }} title="H4 Heading">
                          H4
                        </button>
                      ...
                      {/* H1 Duplicate Hierarchy Warning */}
                      {/(?:^|\n)#[ \t]+[^\n]+/.test(formData.content) && (
                        <div style={{ background: '#FEF3C7', border: '1px solid #FDE68A', borderRadius: 'var(--radius-xs)', padding: '0.5rem 0.75rem', fontSize: '0.75rem', color: '#92400E', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                          <AlertCircle size={14} color="#D97706" />
                          <span><strong>SEO Warning:</strong> Single <code># Heading 1</code> detected inside body. The article title is automatically rendered as the page&apos;s primary H1. Please use <code>## H2</code> and <code>### H3</code> to prevent duplicate H1 tags.</span>
                        </div>
                      )}
                ...
                {activeTab === 'seo' && (
                  <SeoMetadataEditor
                    ...
                    currentId={editingGuide?.id}
                    onAutoGenerate={() => {
                      const auto = generateArticleMetadata(formData);
                      setFormData({
                        ...formData,
                  ...
                  <tr key={g.id} style={{ borderBottom: '1px solid var(--border)' }} className="hover:bg-slate-50">
                    ...
                    <td style={{ padding: '1rem' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', alignItems: 'flex-start' }}>
                        ...
                        <ComplianceBadge scanResult={scanArticle(g)} size="sm" />
                      </div>
                    </td>
