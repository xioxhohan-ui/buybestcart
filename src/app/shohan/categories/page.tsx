...
export default function AdminCategoriesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const generatedSlug = generateCleanSlug(formData.slug || formData.name);
    ...
    if (editingCat) {
      if (editingCat.slug && editingCat.slug !== generatedSlug) {
        await recordSlugChangeRedirect(`/category/${editingCat.slug}`, `/category/${generatedSlug}`);
      }
  ...
  const handleDelete = async (id: string, name: string) => {
    if (confirm(`Delete department/category "${name}"? Any linked products will be unlinked safely.`)) {
      // Safely detach linked products and child subcategories first
          ...
          >
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '1rem' }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '0.625rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-strong)', fontFamily: 'monospace' }}
                  />
                </div>
              </div>

              <SlugUrlAdvisor
                slug={formData.slug || generateCleanSlug(formData.name)}
                sourceTitle={formData.name}
                routePrefix="/category"
                tableName="categories"
                currentId={editingCat?.id}
              ...
              <SeoMetadataEditor
                ...
                rawEntityTitle={formData.name}
                slug={formData.slug || generateCleanSlug(formData.name)}
                pathPrefix="category"
                tableName="categories"
                currentId={editingCat?.id}
                onAutoGenerate={() => {
                  const auto = generateCategoryMetadata(formData);
                  setFormData({
                    ...formData,
