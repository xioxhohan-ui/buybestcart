...
const CurrencyContext = createContext<CurrencyContextType>({
  ...
  setCurrency: () => {},
  setRegion: () => {},
  formatPrice: (amount) => formatPriceHelper(amount, 'USD', FALLBACK_EXCHANGE_RATES),
...
export function CurrencyProvider({ children }: { children: ReactNode }) {
  ...
  const formatPrice = useCallback(
    (amountUsd?: number): string => {
      return formatPriceHelper(amountUsd, currency, rates);
    },
    [currency, rates]
  );
  ...
  const convertPrice = useCallback(
    (amountUsd?: number): number => {
      ...
      const rate = rates[currency] || FALLBACK_EXCHANGE_RATES[currency] || 1.0;
