...
export default function CustomCompareEngine({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
      {/* 01. Quick Comparison Presets Strip */}
      <div style={{ background: '#FAF9F6', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '1.25rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.85rem', flexWrap: 'wrap', gap: '0.5rem' }}>
          <div style={{ fontSize: '0.8125rem', fontWeight: 800, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            <Sparkles size={14} color="var(--green-accent)" />
            <span>Popular Head-to-Head Showdowns</span>
          </div>
        ...
        <div style={{ display: 'flex', gap: '0.625rem', flexWrap: 'wrap' }}>
          <button
            type="button"
            onClick={() => handleQuickPreset(['sony-wh-ch520-wireless-on-ear-bluetooth-headphones-black', 'bose-quietcomfort-ultra-headphones', 'sony-wh-1000xm5-wireless-headphones'])}
            className="btn btn-secondary btn-sm"
            style={{ fontSize: '0.75rem', padding: '0.35rem 0.75rem' }}
          >
            🎧 Sony WH-CH520 vs Bose QC Ultra vs Sony XM5
          </button>
          <button
            type="button"
            onClick={() => handleQuickPreset(['apple-macbook-air-15-m3', 'dell-xps-16-intel-core-ultra-rtx4060'])}
            className="btn btn-secondary btn-sm"
            style={{ fontSize: '0.75rem', padding: '0.35rem 0.75rem' }}
          >
            💻 MacBook Air 15 M3 vs Dell XPS 16 (2-Way)
          </button>
          <button
            type="button"
            onClick={() => handleQuickPreset(['skytech-azure-3-gaming-pc-ryzen-7-9850x3d-rx-9070-xt', 'asus-rog-zephyrus-g16-gaming-laptop'])}
            className="btn btn-secondary btn-sm"
            style={{ fontSize: '0.75rem', padding: '0.35rem 0.75rem' }}
          >
            ⚡ Skytech Gaming PC vs ROG Zephyrus G16
          </button>
        </div>
      </div>
      ...
      {activeProducts.length >= 2 ? (
        <div style={{ overflowX: 'auto', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)' }}>
          <table className="editorial-table" style={{ width: '100%', minWidth: activeProducts.length === 3 ? '860px' : '640px' }}>
            {/* Header: Product Cards & Buy CTAs */}
            <thead>
              <tr>
                <th style={{ width: '22%', background: 'var(--bg-subtle)', verticalAlign: 'middle', padding: '1.5rem 1rem' }}>
                ...
                {activeProducts.map((p) => (
                  <th key={p.id} style={{ width: `${78 / activeProducts.length}%`, textAlign: 'center', padding: '1.75rem 1.25rem', verticalAlign: 'top' }}>
                    {/* Badge */}
                    {p.badge_text && (
                      <div style={{ marginBottom: '0.5rem' }}>
                        <span style={{ background: 'var(--green-accent)', color: '#FFF', fontSize: '0.6875rem', fontWeight: 800, padding: '0.2rem 0.5rem', borderRadius: '4px', textTransform: 'uppercase' }}>
                          {p.badge_text}
                        </span>
                      </div>
                    )}
                    ...
                    <Link href={`/products/${p.slug}`}>
                      <img
                        ...
                        alt={p.title}
                        style={{ height: '120px', maxWidth: '140px', objectFit: 'contain', margin: '0 auto 0.75rem auto', transition: 'transform 0.2s ease' }}
            ...
            <tbody>
              ...
              <tr>
                ...
                {activeProducts.map((p) => (
                  <td key={p.id} style={{ textAlign: 'center', padding: '1rem' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.25rem' }}>
                      {p.editorial_score && (
                        <span style={{ background: 'var(--green-light)', color: 'var(--green-accent)', border: '1px solid var(--green-border)', padding: '0.15rem 0.5rem', borderRadius: 'var(--radius-xs)', fontSize: '0.75rem', fontWeight: 800 }}>
              ...
              <tr>
                ...
                {activeProducts.map((p) => {
                  ...
                  return (
                    <td key={p.id} style={{ padding: '1.25rem 1rem', verticalAlign: 'top' }}>
                      {highlights.length > 0 ? (
                        <ul style={{ margin: 0, paddingLeft: '1.15rem', display: 'flex', flexDirection: 'column', gap: '0.45rem', fontSize: '0.8125rem', color: 'var(--text-secondary)', lineHeight: 1.4 }}>
                          {highlights.map((h: string, i: number) => (
                            <li key={i}>{h}</li>
                          ))}
                        </ul>
                      ) : (
                        <span style={{ color: 'var(--text-muted)', fontSize: '0.8125rem' }}>Verified lab tested</span>
                      )}
                    </td>
                  );
                })}
              ...
              <tr>
                ...
                {activeProducts.map((p) => (
                  <td key={p.id} style={{ textAlign: 'center', fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)', padding: '1rem' }}>
              ...
              <tr>
                ...
                {activeProducts.map((p) => (
                  <td key={p.id} style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', padding: '1rem', lineHeight: 1.5 }}>
              ...
              <tr>
                ...
                {activeProducts.map((p) => (
                  <td key={p.id} style={{ padding: '1rem', verticalAlign: 'top' }}>
                    <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.35rem', fontSize: '0.8125rem' }}>
                      {p.pros && p.pros.length > 0 ? (
                        p.pros.map((pro, i) => (
                          <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.35rem', color: 'var(--text-primary)' }}>
                  ...
                  <td key={p.id} style={{ padding: '1rem', verticalAlign: 'top' }}>
                    <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.35rem', fontSize: '0.8125rem' }}>
                      {p.cons && p.cons.length > 0 ? (
                        p.cons.map((con, i) => (
                          <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.35rem', color: 'var(--text-secondary)' }}>
              ...
              {uniqueSpecKeys.map((specKey) => (
                <tr key={specKey}>
                  ...
                  {activeProducts.map((p) => {
                    ...
                    return (
                      <td key={p.id} style={{ textAlign: 'center', fontSize: '0.8125rem', color: 'var(--text-primary)', padding: '0.85rem' }}>
              ...
              <tr>
                <th style={{ background: 'var(--bg-subtle)', verticalAlign: 'middle' }}>
                  <span style={{ fontWeight: 800, fontSize: '0.8125rem' }}>Check Live Price</span>
                </th>
                {activeProducts.map((p) => (
                  <td key={p.id} style={{ textAlign: 'center', padding: '1.25rem 1rem' }}>
                    <AffiliateCTA
                      productSlug={p.slug}
                      asin={p.asin}
                      price={p.price}
        ...
        <div style={{ textAlign: 'center', background: '#FAF9F6', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '3.5rem 1.5rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', maxWidth: '480px', margin: '0 auto 1.5rem auto', fontSize: '0.9375rem', lineHeight: 1.5 }}>
            Choose products using the search slots above or pick one of our curated showdown presets to view the side-by-side technical breakdown.
          </p>
          <button
            type="button"
            onClick={() => handleQuickPreset(['sony-wh-ch520-wireless-on-ear-bluetooth-headphones-black', 'bose-quietcomfort-ultra-headphones'])}
