              ...
              >
                ...
                {isExpanded && (
                  <div style={{ padding: '1.5rem', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '1.25rem', background: '#FAFAF9' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.75rem' }}>
                      <div>
                        ...
                        <input
                          ...
                          value={item.price ?? ''}
                          onChange={(e) => handleUpdateItem(idx, 'price', e.target.value ? parseFloat(e.target.value) : undefined)}
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '0.6875rem', fontWeight: 800, textTransform: 'uppercase', marginBottom: '0.25rem' }}>
                          List Price ($ USD) <span style={{ fontWeight: 400, color: 'var(--text-muted)' }}>(Optional)</span>
                        </label>
                        <input
                          type="number"
                          step="0.01"
                          ...
                          onChange={(e) => handleUpdateItem(idx, 'list_price', e.target.value ? parseFloat(e.target.value) : undefined)}
                    ...
                    {/* Embed Image HTML / Image Link Auto-Detector */}
                    <EmbedImageInput
                      label="Embed Image HTML / Image Link"
                      placeholder="Paste Amazon affiliate embed HTML (e.g. <a href=...><img src=...></a>) or direct image URL..."
                      defaultAltText={item.title}
                      currentImageUrl={item.thumbnail_url}
                      compact={true}
                      onSelectPrimaryImage={(url, alt, affiliateUrl, asin) => {
                        handleUpdateItem(idx, 'thumbnail_url', url);
                        if (affiliateUrl && !item.affiliate_url) {
                          handleUpdateItem(idx, 'affiliate_url', affiliateUrl);
                        }
                        if (asin && !item.asin) {
                          handleUpdateItem(idx, 'asin', asin);
                        }
                      }}
                      onAddToGallery={(items) => {
                        const existing = item.gallery_images || [];
                        const newUrls = items.map((it) => it.url).filter((u) => !existing.includes(u));
                        if (newUrls.length > 0) {
                          handleUpdateItem(idx, 'gallery_images', [...existing, ...newUrls]);
                        }
                      }}
                    />
