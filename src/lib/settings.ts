...
export interface SiteConfiguration extends BrandSettings, ThemeTokens {
  ...
  amazon_associate_statement: string;
  disclosure_text: string;
  link_level_disclosure: string;
  affiliate_disclosure_page_content?: string;
  feature_flags: {
    dark_mode: boolean;
...
export const DEFAULT_SITE_CONFIG: SiteConfiguration = {
  ...
  disclosure_text:
    'Buy Best Cart is an independent, reader-supported publication. When you buy through links on our site, we may earn an affiliate commission from Amazon and other verified retail partners at no extra cost to you. Amazon, Amazon.com, and the Amazon logo are trademarks of Amazon.com, Inc. or its affiliates.',
  link_level_disclosure: '',
  affiliate_disclosure_page_content: '',
  feature_flags: {
    dark_mode: false,
    price_tracking: true,
    multi_region: true,
  ...
  try {
    ...
    data.forEach((row) => {
      if (row.key === 'general' && row.value) {
        const { admin_path, admin_route, secret, password, ...safeGeneral } = row.value;
        Object.assign(config, safeGeneral);
      }
