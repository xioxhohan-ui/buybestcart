...
export function convertCatalogProductToTopProduct(p: Product, rank: number): TopProductItem {
  ...
  return {
    ...
    affiliate_url: isUrlShortener(p.affiliate_url || '')
      ? (p.asin ? `https://www.amazon.com/dp/${p.asin}?tag=bestbuycart-20` : 'https://www.amazon.com?tag=bestbuycart-20')
      : p.affiliate_url || (p.asin ? `https://www.amazon.com/dp/${p.asin}?tag=bestbuycart-20` : 'https://www.amazon.com?tag=bestbuycart-20'),
    buy_url: isUrlShortener(p.affiliate_url || '')
      ? (p.asin ? `https://www.amazon.com/dp/${p.asin}?tag=bestbuycart-20` : 'https://www.amazon.com?tag=bestbuycart-20')
      : p.affiliate_url || (p.asin ? `https://www.amazon.com/dp/${p.asin}?tag=bestbuycart-20` : 'https://www.amazon.com?tag=bestbuycart-20'),
    video_url: '',
    video_title: '',
...
export function parseProductJsonToTopProducts(jsonString: string, startingRank = 1): TopProductItem[] {
  try {
    ...
    return items.map((raw, idx) => {
      ...
      const rawAffiliate = raw.affiliate_url || raw.buy_url || '';
      const cleanAffiliate = isUrlShortener(rawAffiliate)
        ? (raw.asin ? `https://www.amazon.com/dp/${raw.asin}?tag=bestbuycart-20` : 'https://www.amazon.com?tag=bestbuycart-20')
        : rawAffiliate || (raw.asin ? `https://www.amazon.com/dp/${raw.asin}?tag=bestbuycart-20` : 'https://www.amazon.com?tag=bestbuycart-20');
...
/**
 * Formats raw availability strings (e.g. "in_stock", "out_of_stock", "limited_stock")
 * into user-facing polished display labels ("In Stock", "Out of Stock", "Limited Stock").
 */
export function formatAvailability(availability?: string | null): string {
  if (!availability) return 'In Stock';
  const clean = availability.trim();
  if (!clean) return 'In Stock';

  const lower = clean.toLowerCase();
  if (lower === 'in_stock' || lower === 'instock' || lower === 'in stock') return 'In Stock';
  if (lower === 'out_of_stock' || lower === 'outofstock' || lower === 'out of stock') return 'Out of Stock';
  if (lower === 'limited_stock' || lower === 'low_stock' || lower === 'limited stock') return 'Limited Stock';
  if (lower === 'pre_order' || lower === 'preorder' || lower === 'pre order') return 'Pre-Order';
  if (lower === 'backorder' || lower === 'backordered') return 'Backordered';
  ...
  // If starts with lowercase, capitalize first letter of each word
